...
class KillAura : Module() {
    ...
    private val rangeValue = FloatValue("TargetRange", 3.7f, 1f, 8f)
    private val wallRangeValue = FloatValue("WallRange", 3f, 0f, 8f)
    ...
    private val predictValue = BoolValue("PredictAttack", true)
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        val (_, rotation) = RotationUtils.searchCenter(
                ...
                predictValue.get(),
                mc.thePlayer!!.getDistanceToEntityBox(entity) < wallRangeValue.get(),
                maxRange
        ) ?: return false
    ...
    private val maxRange: Float
        get() = max(rangeValue.get(), wallRangeValue.get())

    private fun getRange(entity: IEntity) =
            (if (mc.thePlayer!!.getDistanceToEntityBox(entity) >= wallRangeValue.get()) rangeValue.get() else wallRangeValue.get()) - if (mc.thePlayer!!.sprinting) rangeSprintReducementValue.get() else 0F
